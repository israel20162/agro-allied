...
export default function Checkout() {
  ...
  if (items.length === 0) {
    return (
      <section className="mx-auto max-w-3xl px-4 py-12">
        <h1 className="font-display text-3xl font-bold text-leaf-800">Nothing to check out</h1>
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-8">
      <h1 className="font-display text-3xl font-bold text-leaf-800">Checkout</h1>
      ...
      <div className="card mt-6 p-5">
        ...
        <p className="mt-4 text-sm text-leaf-600">
          After transferring, upload the receipt or type the transaction reference below.
        </p>
      </div>

      <Formik
        initialValues={{ customer_name: '', phone: '', payment_reference: '', note: '' }}
        ...
        onSubmit={async (values, { setSubmitting }) => {
          ...
          try {
            ...
            console.error(error)
            setFailure('We could not save your order. Check your connection and try again.')
          } finally {
            setSubmitting(false)
          }
        }}
      >
        {({ isSubmitting }) => (
          <Form className="card mt-6 space-y-5 p-5">
            ...
            <div>
              <label className="label" htmlFor="payment_reference">Transaction reference (optional)</label>
              <Field id="payment_reference" name="payment_reference" className="input" placeholder="e.g. OPay 1234567890" />
            </div>
            ...
            <div>
              <label className="label" htmlFor="note">Note for the shop (optional)</label>
              <Field id="note" name="note" as="textarea" rows={3} className="input" placeholder="Please pack the beans separately." />
            </div>

            <div className="flex items-center justify-between rounded-xl bg-leaf-50 px-4 py-3">
              <span className="text-leaf-600">Total</span>
              <span className="font-display text-xl font-bold text-leaf-800">{formatNaira(total)}</span>
            </div>
