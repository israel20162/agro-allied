...
export default function AdminOrders() {
  ...
  async function updateStatus(order: Order, status: OrderStatus) {
    ...
    if (status === 'ready' || status === 'almost_ready') {
      window.open(whatsappLink(order.phone, statusMessage(order.order_number, status)), '_blank')
    }
  }

  const todayTotal = orders.reduce((sum, order) => sum + Number(order.total), 0)
    ...
    <AdminLayout>
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="card p-4">
          ...
          <p className="font-display text-2xl font-bold text-leaf-800">{orders.length}</p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-leaf-500">Value</p>
          <p className="font-display text-2xl font-bold text-leaf-800">{formatNaira(todayTotal)}</p>
        </div>
        <div className="card p-4">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        {(['today', 'active', 'all'] as Filter[]).map((name) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-medium capitalize ${
              filter === name && !phoneSearch ? 'bg-leaf-600 text-white' : 'bg-white text-leaf-700'
            }`}
          >
        ...
        <div className="ml-auto flex gap-2">
          ...
          />
          <button type="button" onClick={loadOrders} className="btn-primary shrink-0 py-2">
            Search
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <p className="mt-8 text-leaf-500">Loading orders…</p>
      ) : orders.length === 0 ? (
        <p className="card mt-6 p-6 text-leaf-600">
          No orders here yet. New orders appear as soon as a customer checks out.
        </p>
      ) : (
                ...
                <div className="mt-4 border-t border-leaf-100 pt-4">
                      ...
                      <tbody>
                        {order.order_items.map((item) => (
                          <tr key={item.id} className="border-t border-leaf-50">
                            <td className="py-1.5">
                              {item.product_name}{' '}
                              <span className="text-leaf-400">per {item.unit}</span>
                            </td>
                  ...
                  <div className="mt-4 flex flex-wrap gap-4">
                    {order.shopping_list_url && (
                      <a href={order.shopping_list_url} target="_blank" rel="noreferrer">
                        <img
                          src={order.shopping_list_url}
                          alt="Shopping list"
                          className="h-40 rounded-xl border border-leaf-100 object-cover"
                        />
                        <span className="mt-1 block text-xs text-leaf-500">Shopping list</span>
                      </a>
                    )}
                    {order.receipt_url && (
                      <a href={order.receipt_url} target="_blank" rel="noreferrer">
                        <img
                          ...
                          className="h-40 rounded-xl border border-leaf-100 object-cover"
                        />
                        <span className="mt-1 block text-xs text-leaf-500">Payment receipt</span>
                      </a>
                    )}
                  </div>
